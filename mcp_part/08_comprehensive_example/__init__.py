"""
本案例构建了一个电商平台智能售后客服系统。用户通过自然语言对话即可完成订单查询、退款申请、批量退款等操作。系统根据订单状态自动分级处理：退款经由HITL人工介入确认处理，如果如果订单状态是已签收，进行二次人工确认。
整个案例涵盖了 MCP 多服务聚合、本地与远程工具混用、工具拦截器、Resource 与 Prompt、进度通知与日志、Elicitation 交互确认、HITL 人工审批等核心知识点。
